...
def redrawGame():
    win.blit(bg, (0,0))
    text = font.render('Score: ' + str(my_score), 1, (255,255,255))
...
while run:
    ...
    if cat.visible == True:
        if ox.y - ox.hitbox[1] < cat.hitbox[1] + cat.hitbox[3] and ox.hitbox[1] + ox.hitbox[3] > cat.hitbox[1]:
                if ox.hitbox[0] + ox.hitbox[2] > cat.hitbox[0] and ox.hitbox[0] < cat.hitbox[0] + cat.hitbox[2]:
                    ox.hit()
                    my_score -= 10
